...
@foreach($posts as $post)
  <div class="block-21 mb-4 d-flex">
    ...
    <div class="text">
      <h3 class="heading"><a href="{{ route('posts.show', ['post' => $post->id, 'slug' => Str::slug($post->title, '-')]) }}">{{ $post->title }}</a></h3>
      <div class="meta">
        <div><a href="#"><span class="icon-calendar"></span> {{ \Carbon\Carbon::parse($post->created_at)->format('F. d, Y') }}</a></div>
